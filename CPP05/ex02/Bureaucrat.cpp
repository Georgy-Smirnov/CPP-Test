...
void Bureaucrat::executeForm(const Form& f, bool success) const
{
	if (success)
		std::cout << name << " executes " << f.getName() << std::endl;
	else
		std::cout << name << " cannot executes " << f.getName() << " because rang Bueraucrat - "<< rang << " Grade for execute - " << f.getGradeForExecute() << std::endl;
}
